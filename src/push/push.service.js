...
class PushService {
  ...
  constructor(db, webpush = require('web-push')) {
    this.db = db;
  ...
  async suscribir(usuarioId, suscripcion, agente) {
    ...
    await this.db.query(
      ...
      [usuarioId, endpoint, p256dh, auth, String(agente || '').slice(0, 255)]
    );
  }
      ...
      for (const destino of destinos) {
        try {
          ...
          if (error.statusCode === 404 || error.statusCode === 410)
            await this.db.query('DELETE FROM suscripciones_push WHERE id=?', [destino.id]);
