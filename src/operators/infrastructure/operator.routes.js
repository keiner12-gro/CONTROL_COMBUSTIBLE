...
function crearRutasOperarios(s,db){const r=express.Router();
 ...
 r.post('/operarios',requirePermission('operarios'),async(q,x,n)=>{try{const z=await s.create(q.body);await registrarAuditoria(db,{usuarioId:q.user.id,usuario:q.user.usuario,rol:q.user.rol,accion:'CREAR',modulo:'operarios',registroId:z.id,detalle:z});x.status(201).json(z)}catch(e){n(e)}});
 r.delete('/operarios/:id',requirePermission('operarios'),async(q,x,n)=>{try{await s.remove(q.params.id);await registrarAuditoria(db,{usuarioId:q.user.id,usuario:q.user.usuario,rol:q.user.rol,accion:'ELIMINAR',modulo:'operarios',registroId:q.params.id});x.json({mensaje:'Operario eliminado. Los registros históricos no se modificaron.'})}catch(e){n(e)}});
 return r;}
