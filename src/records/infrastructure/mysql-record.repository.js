...
class MySQLRecordRepository extends RecordRepository{
 ...
 async machineConsumptionStats(inicio,fin){
   const [r]=await this.db.query(`SELECT r.maquina,COUNT(*) AS registros,COALESCE(SUM(r.cantidad),0) AS total_galones,COALESCE(AVG(r.cantidad),0) AS promedio_galones,COALESCE(MAX(r.cantidad),0) AS maximo_galones,COALESCE(t.capacidad_galones,0) AS capacidad_galones FROM registros_combustible r LEFT JOIN tractores t ON UPPER(t.maquina)=UPPER(r.maquina) WHERE r.cierre_dia=0 AND r.fecha BETWEEN ? AND ? AND r.cantidad IS NOT NULL AND r.cantidad>0 GROUP BY r.maquina,t.capacidad_galones ORDER BY total_galones DESC`,[inicio,fin]);
   return r.map(x=>({...x,registros:Number(x.registros),totalGalones:Number(x.total_galones),promedioGalones:Number(x.promedio_galones),maximoGalones:Number(x.maximo_galones),capacidadGalones:Number(x.capacidad_galones)}));
 }
