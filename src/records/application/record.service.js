...
class RecordService{
  ...
  async saveDailyClosing(x){if(!x.m1Final&&!x.m2Final)throw bad('Debes ingresar al menos una lectura final: M1, M2 o ambas.');const estado=await this.repository.getDailyMeterState(x.fecha);if(estado.hayCierreDiaAnterior){if(estado.m1Anterior!==null&&String(x.m1Inicial||'')!==''&&Number(x.m1Inicial)!==Number(estado.m1Anterior))throw bad(`La lectura inicial de M1 debe coincidir con el cierre anterior: ${estado.m1Anterior}.`);if(estado.m2Anterior!==null&&String(x.m2Inicial||'')!==''&&Number(x.m2Inicial)!==Number(estado.m2Anterior))throw bad(`La lectura inicial de M2 debe coincidir con el cierre anterior: ${estado.m2Anterior}.`);x.m1Inicial=estado.m1Anterior??x.m1Inicial;x.m2Inicial=estado.m2Anterior??x.m2Inicial;}if(x.m1Final&&(!x.m1Inicial||Number(x.m1Final)<Number(x.m1Inicial)))throw bad('La lectura final de M1 no puede ser menor que su inicial.');if(x.m2Final&&(!x.m2Inicial||Number(x.m2Final)<Number(x.m2Inicial)))throw bad('La lectura final de M2 no puede ser menor que su inicial.');const cierre=await this.repository.findDailyClosing(x.fecha);if(cierre){const check=!(await this.repository.hasChecklist(x.fecha,cierre.id));await this.repository.updateDailyClosing(cierre.id,x,check);return convertirRegistroParaFrontend(await this.repository.findById(cierre.id));}const check=!(await this.repository.hasChecklist(x.fecha));const id=await this.repository.insert({...x,cierreDia:true,operario:null,cedula:null,maquina:null,horometro:null,cantidad:null,numeroSai:null,firma:null,observaciones:null,fugaBiodiesel:check?x.fugaBiodiesel:null,sistemaElectrico:check?x.sistemaElectrico:null,paradaEmergencia:check?x.paradaEmergencia:null});return convertirRegistroParaFrontend(await this.repository.findById(id));}
