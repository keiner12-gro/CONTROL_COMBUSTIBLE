...
function actualizarConfirmacion(){const el=document.getElementById('confirmacion-registro');if(!el)return;const t=tractoresDisponibles.find(x=>normalizarBusquedaMaquina(x.maquina)===normalizarBusquedaMaquina(maquina.value));el.innerHTML=`<div class="confirmacion-linea"><span>🚜</span><div><small>Máquina</small><strong>${maquina.value||'—'}</strong><em>${t?.descripcion||tractorDescripcion.value||''}</em></div></div><div class="confirmacion-linea"><span>👤</span><div><small>Operario</small><strong>${nombreOperario.value||'—'}</strong><em>Cédula: ${cedulaOperario.value||'—'}</em></div></div><div class="confirmacion-linea"><span>⛽</span><div><small>Cantidad</small><strong>${Number(cantidad.value||0).toFixed(2)} GAL</strong><em>Horómetro: ${horometro.value||'—'}</em></div></div><div class="confirmacion-linea"><span>📄</span><div><small>Soporte</small><strong>${numeroSai.value||'Sin SAI'}</strong><em>${firmaOperario.value?'✓ Firma guardada':'⚠ Falta firma'}</em></div></div>`;}
...
function mostrarDatosTractorSeleccionado() {
  ...
  if (resumen) {
    ...
    if (tractorSeleccionado) resumen.innerHTML = `✓ <strong>${tractorSeleccionado.maquina}</strong> · ${tractorSeleccionado.descripcion || 'Sin descripción'} · Tanque ${Number(tractorSeleccionado.capacidad_galones || tractorSeleccionado.capacidad || 0).toFixed(2)} gal`;
  }
