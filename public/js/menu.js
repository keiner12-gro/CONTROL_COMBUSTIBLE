...
async function cargarNotificaciones(){
 ...
 try{const r=await fetch('/api/notificaciones',{headers:{},cache:'no-store'});if(!r.ok)return;const datos=await r.json();const pendientes=datos.filter(n=>Number(n.leida)===0);if(contador)contador.textContent=String(pendientes.length);if(panel)panel.hidden=pendientes.length===0;const badge=document.getElementById('dashboard-alerta-badge');if(badge){badge.hidden=!pendientes.length;badge.textContent=pendientes.length>9?'9+':String(pendientes.length)}const dash=document.getElementById('dashboard-alertas');if(dash)dash.textContent=String(pendientes.length);if(lista){lista.innerHTML='';pendientes.slice(0,5).forEach(n=>{const d=document.createElement('div');d.className='notificacion-item';d.innerHTML=`<strong>${n.titulo}</strong><p>${n.mensaje}</p>`;const b=document.createElement('button');b.type='button';b.textContent='Ver alerta';b.onclick=()=>window.location.replace('alertas.html');d.appendChild(b);lista.appendChild(d)})}}
