// ============================================================================
// alertas.js — BANDEJA DE ALERTAS (public/html/alertas.html)
// ----------------------------------------------------------------------------
// Muestra todas las alertas generadas por el sistema como tarjetas, con:
//   * tres contadores arriba (total / pendientes / justificadas) que también
//     funcionan como filtro de estado,
//   * chips para filtrar por tipo de alerta,
//   * un cuerpo distinto por cada tipo (barra de capacidad, comparativo de
//     promedio, diferencia de horómetro, etc.),
//   * el diálogo para justificar una alerta y adjuntar un soporte.
// PARA AGREGAR UN TIPO DE ALERTA NUEVO hay que tocar: TIPOS_ALERTA,
// ORDEN_TIPOS, PRIORIDAD_ALERTA y el bloque "cuerpo" de tarjetaAlerta().
// ============================================================================

// Elementos de la pantalla.
const cuerpoAlertas=document.getElementById('cuerpo-alertas'); // Contenedor de tarjetas
const resumenAlertas=document.getElementById('resumen-alertas'); // Contadores superiores
const filtrosAlertas=document.getElementById('filtros-alertas'); // Chips por tipo
const botonActualizar=document.getElementById('boton-actualizar-alertas');
// Cabeceras comunes de los envíos (la identidad va en la cookie de sesión).
function headersSesion(){return {'Content-Type':'application/json'};}

// Catálogo de tipos de alerta: etiqueta e ícono que se muestran en pantalla.
const TIPOS_ALERTA={
...
// Cada contador del resumen es además un botón que filtra por estado.
...
// Aplica los dos filtros (tipo y estado) sobre las alertas descargadas.
function alertasFiltradas(){
  return alertasCargadas.filter(a=>{
    const tipo=a.tipo_alerta||'sobrecapacidad';
    const estado=a.estado==='justificada'?'justificada':'pendiente'; // Solo dos estados visibles
...
// Dibuja los tres contadores y les conecta el filtrado por estado.
function renderizarResumen(){
  ...
  // Al hacer clic se cambia el filtro y se repinta todo.
...
// Dibuja los chips de filtro por tipo, cada uno con su cantidad entre paréntesis.
function renderizarFiltrosTipo(){
  // Cuenta cuántas alertas hay de cada tipo.
  const conteos=ORDEN_TIPOS.reduce((acc,t)=>{acc[t]=alertasCargadas.filter(a=>(a.tipo_alerta||'sobrecapacidad')===t).length;return acc;},{});
  // El chip "Todas" va primero y después uno por cada tipo.
...
// Construye la tarjeta de UNA alerta. El cuerpo central cambia según el tipo.
function tarjetaAlerta(a){
  ...
  // --- Cuerpo variable según el tipo de alerta ---
  let cuerpo='';
  if(tipo==='promedio'){
    // Compara el promedio histórico con lo suministrado y muestra el % de exceso.
    ...
    // Capacidad del tanque vs. galones cargados, con el exceso en galones.
    ...
    // Muestra el valor escrito, el anterior válido y la diferencia si es calculable.
    ...
    // Tipo histórico que ya no se genera.
  ...
  // --- Armado de la tarjeta: insignias, título, operario, cuerpo y observaciones ---
  const card=document.createElement('article');
  card.className=`alerta-card ${estado} tipo-${tipo}`; // Las clases definen los colores
  ...
  // --- Pie de la tarjeta: justificada muestra el soporte; pendiente, el botón ---
  ...
  if(a.estado==='justificada'){
    ...
    // El adjunto se pide al endpoint protegido, nunca por su ruta de archivo.
...
// Vacía el contenedor y dibuja las alertas que pasan los filtros.
...
// Descarga las alertas del servidor y repinta las tres zonas de la pantalla.
...
// Diálogo de justificación: texto obligatorio + archivo opcional (PDF o imagen).
async function justificarAlerta(a){
  ...
  // preConfirm valida antes de cerrar: exige justificación, revisa peso y tipo
  // del archivo y lo convierte a base64 (FileReader) para enviarlo como JSON.
  // Las MISMAS validaciones se repiten en el servidor (alert.service.js).
  const r=await Swal.fire({icon:'warning',title:`Justificar ${a.maquina}`,html,showCancelButton:true,confirmButtonText:'Guardar justificación',cancelButtonText:'Cancelar',preConfirm:async()=>{const just=String(document.getElementById('justificacion-alerta').value||'').trim();const file=document.getElementById('reporte-alerta').files[0];if(!just){Swal.showValidationMessage('La justificación es obligatoria.');return;}if(!file)return {justificacion:just};if(file.size>3*1024*1024){Swal.showValidationMessage('El archivo debe pesar máximo 3 MB.');return;}const permitidos=['application/pdf','image/png','image/jpeg','image/webp'];if(file.type&&!permitidos.includes(file.type)){Swal.showValidationMessage('El archivo debe ser PDF, PNG, JPG o WEBP.');return;}const base64=await new Promise((ok,no)=>{const fr=new FileReader();fr.onload=()=>ok(fr.result);fr.onerror=no;fr.readAsDataURL(file);});return {justificacion:just,reporteBase64:base64,reporteNombre:file.name,reporteTipo:file.type};}});
  // Si se confirmó, se envía al servidor y se recarga la bandeja.
...
// Botón de refrescar manual y carga inicial de la pantalla.
